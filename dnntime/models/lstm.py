...
class LSTMWrapper:
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, n_epoch: int = 10,
        ...
        """
        Wraps the LSTM model.fit() function and includes the time it takes to
        finish running.
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray, score_type: str = 'rmse',
        ...
        Returns
        -------
        self.lstm_model : The trained LSTM model itself.
        lstm_pred : The LSTM's forecasted data or y_hat based on X_test.
...
def StackedLSTM(n_input: int, n_output: int, n_unit: int, n_feature: int,
    ...
    """
    A standard, 3-layer deep LSTM model that includes dropout rates.
